...
defineOptions({ name: 'AGCMSettingsSubmittalTypes' });

const BASE = '/agcm_submittal';
const ENTITY_NAME = 'Submittal Type';
...
async function fetchData() {
  loading.value = true;
  try {
    const data = await requestClient.get(`${BASE}/submittal-types`);
...
async function handleSave() {
  ...
  try {
    if (editingId.value) {
      await requestClient.put(`${BASE}/submittal-types/${editingId.value}`, { name: formName.value });
      message.success(`${ENTITY_NAME} updated`);
    } else {
      await requestClient.post(`${BASE}/submittal-types`, { name: formName.value });
...
async function handleDelete(record) {
  try {
    await requestClient.delete(`${BASE}/submittal-types/${record.id}`);
