...
class TestSchedule:
    ...
    def test_schedule_sequence(self, db, load_model, project_ids, company_id):
        ...
        scheds = db.query(Schedule).filter(Schedule.project_id == project_ids[0]).order_by(Schedule.id).all()
    ...
    def test_activate_schedule(self, db, load_model, project_ids, company_id):
        ...
        s1 = Schedule(name="S1", project_id=project_ids[0], company_id=company_id, is_active=True)
        s2 = Schedule(name="S2", project_id=project_ids[0], company_id=company_id, is_active=False)
        db.add_all([s1, s2])
        db.flush()
    ...
    def test_delete_schedule_cascades(self, db, load_model, project_ids, company_id):
        ...
        sched = Schedule(name="Cascade sched", project_id=project_ids[0], company_id=company_id)
        db.add(sched)
        db.flush()

        wbs = WBS(code="1.0", name="Foundation", schedule_id=sched.id, project_id=project_ids[0], company_id=company_id)
        db.add(wbs)
        db.flush()

        task = Task(name="Excavation", schedule_id=sched.id, project_id=project_ids[0], company_id=company_id, wbs_id=wbs.id)
        db.add(task)
        db.flush()
        wbs_id = wbs.id
        task_id = task.id
...
class TestWBS:
    ...
    def test_create_wbs(self, db, load_model, project_ids, company_id):
        ...
        sched = Schedule(name="WBS sched", project_id=project_ids[0], company_id=company_id)
        db.add(sched)
        db.flush()

        wbs = WBS(code="1.0", name="Sitework", schedule_id=sched.id, project_id=project_ids[0], company_id=company_id)
        db.add(wbs)
        db.flush()
    ...
    def test_wbs_hierarchy(self, db, load_model, project_ids, company_id):
        ...
        sched = Schedule(name="Hierarchy sched", project_id=project_ids[0], company_id=company_id)
        db.add(sched)
        db.flush()

        parent = WBS(code="1.0", name="Foundation", schedule_id=sched.id, project_id=project_ids[0], company_id=company_id)
        db.add(parent)
        db.flush()

        child = WBS(code="1.1", name="Excavation", schedule_id=sched.id, project_id=project_ids[0], company_id=company_id, parent_id=parent.id)
        db.add(child)
        db.flush()
...
class TestTask:
    ...
    def _make_schedule(self, db, load_model, project_ids, company_id):
        Schedule = _schedule(load_model)
        sched = Schedule(name="Task sched", project_id=project_ids[0], company_id=company_id)
        db.add(sched)
        db.flush()
        return sched
    ...
    def test_task_sequence(self, db, load_model, project_ids, company_id):
        ...
        tasks = db.query(Task).filter(Task.schedule_id == sched.id).order_by(Task.id).all()
    ...
    def test_task_progress_update(self, db, load_model, project_ids, company_id):
        ...
        task = Task(name="Progress task", schedule_id=sched.id, project_id=project_ids[0], company_id=company_id, progress=0)
        db.add(task)
        db.flush()
    ...
    def test_list_tasks_filter_by_status(self, db, load_model, project_ids, company_id):
        ...
        db.add(Task(name="T1", schedule_id=sched.id, project_id=project_ids[0], company_id=company_id, status=TaskStatus.TODO))
        db.add(Task(name="T2", schedule_id=sched.id, project_id=project_ids[0], company_id=company_id, status=TaskStatus.IN_PROGRESS))
        db.add(Task(name="T3", schedule_id=sched.id, project_id=project_ids[0], company_id=company_id, status=TaskStatus.TODO))
        db.flush()

        todos = db.query(Task).filter(Task.status == TaskStatus.TODO, Task.schedule_id == sched.id).all()
        assert len(todos) == 2
...
class TestTaskDependency:
    ...
    def test_create_dependency(self, db, load_model, project_ids, company_id):
        ...
        sched = Schedule(name="Dep sched", project_id=project_ids[0], company_id=company_id)
        db.add(sched)
        db.flush()

        t1 = Task(name="Predecessor", schedule_id=sched.id, project_id=project_ids[0], company_id=company_id)
        t2 = Task(name="Successor", schedule_id=sched.id, project_id=project_ids[0], company_id=company_id)
        db.add_all([t1, t2])
        db.flush()
    ...
    def test_gantt_data(self, db, load_model, project_ids, company_id):
        ...
        sched = Schedule(name="Gantt sched", project_id=project_ids[0], company_id=company_id)
        db.add(sched)
        db.flush()

        wbs = WBS(code="1.0", name="Phase 1", schedule_id=sched.id, project_id=project_ids[0], company_id=company_id)
        db.add(wbs)
        db.flush()

        t1 = Task(name="Gantt T1", schedule_id=sched.id, project_id=project_ids[0], company_id=company_id, wbs_id=wbs.id, planned_start=date(2026, 4, 1), planned_end=date(2026, 4, 5))
        t2 = Task(name="Gantt T2", schedule_id=sched.id, project_id=project_ids[0], company_id=company_id, wbs_id=wbs.id, planned_start=date(2026, 4, 6), planned_end=date(2026, 4, 10))
        db.add_all([t1, t2])
        db.flush()

        dep = TaskDependency(predecessor_id=t1.id, successor_id=t2.id, dependency_type=DependencyType.FS, company_id=company_id)
        db.add(dep)
        db.flush()
